...
merge_codex_hooks_json() {
    ...
    if ! command -v python3 >/dev/null 2>&1; then
        print_warning "python3 not found; skipping Codex hooks merge for $hooks_path"
        return 1
    fi

    python3 - "$hooks_path" "$source_path" <<'PY'
import json
import os
import sys

hooks_path, source_path = sys.argv[1], sys.argv[2]
...
merge_codex_marketplace_json() {
    ...
    if ! command -v python3 >/dev/null 2>&1; then
        print_warning "python3 not found; skipping Codex marketplace merge for $marketplace_path"
        return 1
    fi

    python3 - "$marketplace_path" "$plugin_name" "$plugin_path" <<'PY'
import json
import os
import sys

marketplace_path, plugin_name, plugin_path = sys.argv[1], sys.argv[2], sys.argv[3]
...
marketplace.setdefault("name", "manifold-personal")
marketplace.setdefault("interface", {}).setdefault("displayName", "Manifold Personal Plugins")
...
CODEX_AGENTS_DIR="agents/codex/agents"
CODEX_HOOKS_FILE="agents/codex/hooks.json"
CODEX_PLUGIN_DIR="plugins/manifold-codex"
CLAUDE_AGENTS_DIR="agents/claude"
...
install_manifold_native() {
    ...
    # Install custom agents used by model-routed commands
    local agent_count=0
    if [[ -n "$LOCAL_INSTALL" ]]; then
        if [[ -d "$SCRIPT_DIR/$CLAUDE_AGENTS_DIR" ]]; then
            for agent_file in "$SCRIPT_DIR/$CLAUDE_AGENTS_DIR"/*.md; do
        ...
        local claude_agents=(
            ...
            "m5-verify-worker.md"
            "m6-integrate-worker.md"
        )
        for agent_file in "${claude_agents[@]}"; do
            if curl -fsSL "$REPO/install/$CLAUDE_AGENTS_DIR/$agent_file" -o "$agents_dir/$agent_file" 2>/dev/null; then
                ((agent_count++))
            fi
        done
    fi
    print_success "Installed $agent_count custom agents to $agents_dir/"
...
install_manifold_codex() {
    ...
    # Install global Codex custom agents
    local agents_dir="$codex_config_dir/agents"
    mkdir -p "$agents_dir"
    local agent_count=0
    if [[ -n "$LOCAL_INSTALL" ]]; then
        if [[ -d "$SCRIPT_DIR/$CODEX_AGENTS_DIR" ]]; then
            for agent_file in "$SCRIPT_DIR/$CODEX_AGENTS_DIR"/*.toml; do
                [[ -f "$agent_file" ]] || continue
                cp "$agent_file" "$agents_dir/"
                ((agent_count++))
            done
        ...
        local codex_agents=(
            "manifold-constraint-discovery.toml"
            "manifold-tension-analysis.toml"
            "manifold-anchor-reasoner.toml"
            "manifold-generation-worker.toml"
            "manifold-verification-reviewer.toml"
            "manifold-integration-worker.toml"
            "manifold-parallel-planner.toml"
            "manifold_m0_init_worker.toml"
            "manifold_m_status_worker.toml"
            "manifold_m_solve_worker.toml"
            "manifold_m4_generate_worker.toml"
            "manifold_m5_verify_worker.toml"
            "manifold_m6_integrate_worker.toml"
        )
        for agent_file in "${codex_agents[@]}"; do
            if curl -fsSL "$REPO/install/$CODEX_AGENTS_DIR/$agent_file" -o "$agents_dir/$agent_file" 2>/dev/null; then
                ((agent_count++))
            fi
        done
    fi
    print_success "Installed $agent_count Codex custom agents to $agents_dir/"

    # Install and merge global Codex hooks.json
    local hooks_path="$codex_config_dir/hooks.json"
    local manifold_hooks_tmp="/tmp/manifold-codex-hooks-$$.json"
    if [[ -n "$LOCAL_INSTALL" ]]; then
        cp "$SCRIPT_DIR/$CODEX_HOOKS_FILE" "$manifold_hooks_tmp"
    else
        curl -fsSL "$REPO/install/$CODEX_HOOKS_FILE" -o "$manifold_hooks_tmp" 2>/dev/null || true
    fi
    if [[ -f "$manifold_hooks_tmp" ]]; then
        if merge_codex_hooks_json "$hooks_path" "$manifold_hooks_tmp"; then
            print_success "Installed Codex global hooks to $hooks_path"
        else
            print_warning "Could not merge Codex hooks into $hooks_path"
        fi
        rm -f "$manifold_hooks_tmp"
    else
        print_warning "Could not fetch Codex hooks.json"
    fi

    # Install Codex plugin bundle and add personal marketplace entry
    local plugin_root="$codex_config_dir/plugins"
    local plugin_dir="$plugin_root/manifold-codex"
    mkdir -p "$plugin_root"
    rm -rf "$plugin_dir"
    if [[ -n "$LOCAL_INSTALL" ]]; then
        if [[ -d "$SCRIPT_DIR/../$CODEX_PLUGIN_DIR" ]]; then
            cp -R "$SCRIPT_DIR/../$CODEX_PLUGIN_DIR" "$plugin_dir"
            print_success "Installed Codex plugin bundle to $plugin_dir"
        fi
    else
        mkdir -p "$plugin_dir/.codex-plugin" "$plugin_dir/commands" "$plugin_dir/skills"

        curl -fsSL "$REPO/$CODEX_PLUGIN_DIR/.codex-plugin/plugin.json" -o "$plugin_dir/.codex-plugin/plugin.json" 2>/dev/null || true
        curl -fsSL "$REPO/$CODEX_PLUGIN_DIR/README.md" -o "$plugin_dir/README.md" 2>/dev/null || true
        curl -fsSL "$REPO/$CODEX_PLUGIN_DIR/hooks.json" -o "$plugin_dir/hooks.json" 2>/dev/null || true

        for cmd_file in "${COMMAND_FILES[@]}"; do
            local cmd_name="${cmd_file%.md}"
            [[ "$cmd_name" == SCHEMA_REFERENCE ]] && continue
            curl -fsSL "$REPO/$CODEX_PLUGIN_DIR/commands/$cmd_file" -o "$plugin_dir/commands/$cmd_file" 2>/dev/null || true
            mkdir -p "$plugin_dir/skills/manifold-${cmd_name}"
            curl -fsSL "$REPO/$CODEX_PLUGIN_DIR/skills/manifold-${cmd_name}/SKILL.md" \
                -o "$plugin_dir/skills/manifold-${cmd_name}/SKILL.md" 2>/dev/null || true
        done
        print_success "Installed Codex plugin bundle to $plugin_dir"
    fi
    ...
    if merge_codex_marketplace_json "$marketplace_path" "manifold-codex" "./.codex/plugins/manifold-codex"; then
        print_success "Updated Codex personal marketplace at $marketplace_path"
    else
        print_warning "Could not update Codex personal marketplace at $marketplace_path"
    fi
...
validate_installation() {
    ...
    # Check worker agents for model-routed commands
    local worker_agents=(
        "m0-init-worker.md"
        "m-status-worker.md"
        "m-solve-worker.md"
    ...
    for agent_file in "${worker_agents[@]}"; do
        if [[ ! -f "$agents_dir/$agent_file" ]]; then
            print_warning "Missing: agents/$agent_file"
        else
            print_success "Found: agents/$agent_file"
        fi
    done
...
validate_codex_installation() {
    ...
    # Check global custom agents
    local agent_count=0
    if [[ -d "$agents_dir" ]]; then
        agent_count=$(ls "$agents_dir"/*.toml 2>/dev/null | wc -l | tr -d ' ')
    fi
    if [[ "$agent_count" -gt 0 ]]; then
        print_success "Found: $agent_count Codex custom agents"
    else
        print_warning "Missing: Codex custom agents in $agents_dir/"
    fi

    # Check global hooks
    if [[ -f "$hooks_path" ]]; then
        print_success "Found: Codex hooks.json"
    else
        print_warning "Missing: $hooks_path"
    fi

    # Check Codex plugin bundle + marketplace
    if [[ -f "$plugin_dir/.codex-plugin/plugin.json" ]]; then
        print_success "Found: Codex plugin bundle"
    else
        print_warning "Missing: $plugin_dir/.codex-plugin/plugin.json"
    fi

    if [[ -f "$marketplace_path" ]] && grep -q '"manifold-codex"' "$marketplace_path" 2>/dev/null; then
        print_success "Found: Codex personal marketplace entry"
    else
        print_warning "Missing: Codex marketplace entry in $marketplace_path"
    fi
