...
class ParallelExecutor2 extends EventEmitter {
  ...
  async runAgentInWorktree(taskId, worktree) {
    return new Promise((resolve) => {
      ...
      const process2 = spawn(this.config.agentCommand, ["--print", "--no-tty", taskDescription], {
        cwd: worktree.path,
        shell: true,
        env: {
          ...process2.env,
      ...
      this.runningProcesses.set(taskId, process2);
...
class ProgressReporter extends EventEmitter2 {
  config;
  state;
  progressIntervalId;
  constructor(config = {}) {
...
Cleaning up worktrees...`);
  ...
  complete(success) {
    ...
    this.addUpdate(success ? "cleanup" : "execution", success ? "Parallel execution completed" : "Parallel execution failed");
...
`);
  ...
  addUpdate(phase, message, details) {
  ...
  calculateProgress() {
    if (this.state.totalTasks === 0)
      return 100;
    return Math.round((this.state.completedTasks + this.state.failedTasks) / this.state.totalTasks * 100);
  }
...
var CONFIG_FILENAME = ".parallel.yaml";
...
`);
  ...
  validate() {
    ...
    if (this.config.maxParallel < 1 || this.config.maxParallel > 10) {
      errors2.push("maxParallel must be between 1 and 10");
    }
    if (this.config.maxDiskUsagePercent < 50 || this.config.maxDiskUsagePercent > 95) {
      errors2.push("maxDiskUsagePercent must be between 50 and 95");
    }
    if (this.config.maxMemoryUsagePercent < 50 || this.config.maxMemoryUsagePercent > 95) {
      errors2.push("maxMemoryUsagePercent must be between 50 and 95");
    }
    if (this.config.maxCpuLoadPercent < 50 || this.config.maxCpuLoadPercent > 95) {
      errors2.push("maxCpuLoadPercent must be between 50 and 95");
    }
    if (this.config.timeout < 1e4 || this.config.timeout > 3600000) {
      errors2.push("timeout must be between 10 seconds and 1 hour");
    }
    if (!["sequential", "squash", "rebase"].includes(this.config.mergeStrategy)) {
      errors2.push("mergeStrategy must be sequential, squash, or rebase");
    }
    return {
  ...
  toString() {
    return $stringify(this.config, { indent: 2 });
  }
  isEnabled() {
...
function parseParallelFlags(args) {
  const flags = {};
